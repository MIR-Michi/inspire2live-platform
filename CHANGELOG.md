...
### Documentation
- **Channel syndication concept (proposal, nothing implemented).** [`docs/CHANNEL_SYNDICATION_CONCEPT.md`](docs/CHANNEL_SYNDICATION_CONCEPT.md) designs the "draft a LinkedIn post from this page" capability as a reusable component rather than a button: a new `syndication` component owns drafts, channel profiles and the AI workload; each entity-owning component publishes a curated `PublishableSource` through a kernel `SourceProvider` contract and a new optional `provides.sources` manifest field (reconciled in CI); composition happens in one top-level registry file so `syndication` never depends on source owners; a channel is data plus an optional connector, never a module. A human-approved draft hands over to the existing `content_calendar` through `content`'s own action, and provenance is written back to `campus_sessions.published_outputs` through the provider — no second publishing path, one new table, no LinkedIn API in the first slice. First adopter is the World Campus session page. Decision to be recorded as ADR-0014 if accepted; requirements tracked as `REQ-SYN-001…004` (`planned`). See `docs/changes/2026-08-19-channel-syndication-concept.md`.
