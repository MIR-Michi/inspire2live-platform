...
- **Conference discovery is configurable + the manual "Refresh cache" button is gone from the conferences page.** A new **Events & Conferences** panel in Platform Settings exposes: automatic discovery on/off, minimum days between searches (the daily cron honours it), look-ahead window (months), max web searches per lane, source lenses per region, and the known-conference hint size. The conferences page shows a read-only last-refresh status with a shortcut to the panel; on-demand refresh remains available to admins via the existing endpoint.
