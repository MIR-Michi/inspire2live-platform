...
|---|---|---|---|---|
| S14-T01 | Add `@anthropic-ai/sdk`; keep `ANTHROPIC_API_KEY` in `.env.example`, `README.md`, `docs/ENVIRONMENT_REFERENCE.md` as a fallback/bootstrap default | TBD | Not Started | Admin settings (T02–T03) are the primary config source |
| S14-T02 | `ai_settings` migration — single org-wide record (encrypted `api_key`, `model`, `effort`, `updated_by`, `updated_at`) + admin-only RLS + server-only encryption helper (app-level secret or Supabase Vault) | TBD | Not Started | Key encrypted at rest, never returned to the client |
...
| S14-T04 | `src/lib/ai/client.ts` — singleton client; resolve config from `ai_settings` (key/model/effort) with env-key fallback; helpers for structured output (Zod → `output_config.format`), web search, adaptive thinking, timeout/retry, typed errors; validate model↔effort pairing | TBD | Not Started | One wrapper all features call; no direct SDK use elsewhere |
...
| S14-T07 | `docs/AI_INTEGRATION.md` — patterns, guardrails, secret handling, model/reasoning settings, untrusted-input/prompt-injection policy, model-per-workload table, citation rule | TBD | Not Started | The "how we use Claude" reference |
