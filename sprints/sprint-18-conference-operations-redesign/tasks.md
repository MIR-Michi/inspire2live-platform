...
| S18-T08 | **Guest-scoped operating page.** Guest edits the operating-page surface (tiles / phase / traffic-light) under token auth. | TBD | In Progress | Experience unified via the shared design + the existing token RPCs (`get_guest_workspace`, `add_guest_note`, `register_guest_file`, …). The deeper single-record storage merge onto `conference_prep` is T10. |
...
| S18-T10 | **Guest/internal data reconciliation.** Fold guest submission/notes/files into the operating record; retire the read-only guest-reports block. | TBD | Deferred | Concept §6, §9. Guest data still lives in `conference_guest_*`; surfaced to the team as before. Storage merge is the next follow-up. |
...
| S18-T14 | **Verification.** Typecheck, lint, unit green; smoke both paths. | TBD | In Progress | `pnpm typecheck` + `pnpm lint` + `pnpm test` (489) green. E2E and live-DB smoke of the guest path pending a DB with migration `00162` applied. |
