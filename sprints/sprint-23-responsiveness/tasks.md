...
| S23-T01b | **Decide what local development points at.** `.env.local` sets `NEXT_PUBLIC_SUPABASE_URL` to the **cloud** Supabase project, not the local stack that `AGENTS.md` documents. So every page load in `pnpm dev` pays a real transatlantic-class round trip over home broadband (~40–88 ms measured, ×11–16 per page), which means local development may *feel* far worse than production does and is a poor signal for this sprint's measurements. | TBD | Not Started | Found while probing T01. Two separate questions: whether the perceived slowness is partly a local-development artefact (affects how T03 baselines are taken — take them against a deployment, not `localhost`), and whether developing against a shared cloud database is intended at all. If it is a production project, that is a data-safety question outside this sprint — raise it, do not fix it here. |
