...
| REQ-CONF-OPS-003 | Unified guest experience — guest workspace adopts the phase header, traffic-light status, and an overview-first card grid of the guest's conferences; presentation only for presenters; photos requested only during/after | `done` (experience) | `src/app/congress/attend/[token]/workspace/guest-workspace.tsx` | Reuses existing token RPCs; single-record storage merge onto `conference_prep` deferred (S18-T08/T09/T10) |
