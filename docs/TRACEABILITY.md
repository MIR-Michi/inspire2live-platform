...
| REQ-PUB-005 | Human approval is unconditional and auditable, and approved copy hands over rather than dead-ends — `pending → approved → published` with supersede-on-regenerate, untouched `ai_body` kept beside the edited text, `approved_by`/`approved_at` recorded, no setting able to disable the gate, and handover through `content`'s own calendar action with provenance written back by the provider | Concept §4.4, §6 · ADR-0014 §6, §8 | `done` | `src/modules/publishing/domain/lifecycle.ts`, `src/modules/content/domain/calendar-entries.ts`, `src/modules/events/domain/publishing-sources.ts` (`onPublished`), `supabase/migrations/00173_publishing_component.sql` (one-live-run index), `src/test/unit/publishing-lifecycle.test.ts` | 2026-08-19 | Mirrors `intake_ai_suggestions`; only publication-intended fields ever leave the owning component. Approval-before-handover and the rights block are enforced in the domain and unit-tested; the resulting `content_calendar` row has not yet been observed |
