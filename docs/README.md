...
| [PODCAST_RADAR_CONCEPT.md](PODCAST_RADAR_CONCEPT.md) | Phase B concept (built in Sprint 22): assisted discovery of questions and guests — one reviewable proposal per card, structured open APIs as the spine with the model only grouping and phrasing, and promotion into draft questions and wishlist cards |
| [PERFORMANCE_AND_RESPONSIVENESS_CONCEPT.md](PERFORMANCE_AND_RESPONSIVENESS_CONCEPT.md) | Sprint 23 concept: why the platform feels laggy and what "responsive" means numerically — the four compounding causes (region distance, no request-scoped memoisation, the serialised Server Action queue, whole-route revalidation), the per-interaction budgets, and the structural gates that keep them |
