...
export default async function CommsPodcastPage({
  ...
  searchParams?: Promise<{ tab?: string; show?: string; screen?: string; card?: string }>
}) {
  const params = (await searchParams) ?? {}
  // Sprint 20 (ADR-0013): the Guests roster is replaced by the Planning &
  // Strategy planner. Past guests now live in the People list, where they are
  // the strongest introducers available for the next question.
  const activeTab = params.tab === 'planning' ? 'planning' : 'episodes'
  const showAll = params.show === 'all'

  if (activeTab === 'planning') {
    const screen = PLANNING_SCREENS.includes(params.screen as PlanningScreen)
      ? (params.screen as PlanningScreen)
      : 'board'
    return (
      <section className="mx-auto max-w-6xl space-y-5">
        <PodcastHeader activeTab="planning" />
        <PlanningStrategyShell
          screen={screen}
          cardId={params.card}
          basePath="/app/comms/podcast?tab=planning"
        />
      </section>
    )
  }
  ...
  return (
    <section className="mx-auto max-w-5xl space-y-5">
      <PodcastHeader activeTab="episodes" people={people} />
...
/**
 * Page header + tab bar, shared by both tabs.
 *
 * Sprint 20: the second tab is **Planning & Strategy**, not Guests. A guest
 * roster is a record of the past and already lives on the episode record; the
 * booking work — choosing the question, finding who can answer it, and finding a
 * route to them — is what was missing. Past guests moved into the planner's
 * People list (concept §1, ADR-0013).
 */
function PodcastHeader({
  activeTab,
  people,
}: {
  activeTab: 'episodes' | 'planning'
