...
import {
  createEventChecklistTask,
  deleteEventChecklistTask,
  seedEventChecklist,
  updateEventChecklistTask,
...
/**
 * The editable checklist for a podcast episode. Each task is a comms_task tied
 * to the event (so it also appears on its owner's personal dashboard). Owners,
 * titles, due dates and status are editable; tasks can be added or deleted. An
 * empty episode can be seeded with the standard podcast template.
 *
 * Mirrors the Campus meeting checklist so the two workspaces feel identical.
 */
export function EventTaskChecklist({
  eventId,
  tasks,
  teamMembers,
}: {
  ...
  const changeStatus = (task: CommsTaskRecord, value: string) => {
    ...
    run(updateEventChecklistTask, { task_id: task.id, status: value })
  }
  ...
  const seed = () => run(seedEventChecklist, {})
  const addTask = () => {
  ...
  const saveTitle = (task: CommsTaskRecord) => {
    ...
    run(updateEventChecklistTask, { task_id: task.id, title }, () => setEditingId(null))
  }
          ...
          >
            ...
            {editingId === task.id ? (
              <input
                ...
                autoFocus
                className="min-w-0 flex-1 rounded-lg border border-orange-300 px-2 py-1 text-sm focus:outline-none"
              />
            ) : (
              <button
                type="button"
                ...
                className={`min-w-0 flex-1 truncate text-left text-sm font-medium transition-colors hover:text-orange-800 ${task.status === 'completed' ? 'text-neutral-400' : 'text-neutral-900'}`}
                title="Click to edit"
              >
            ...
            <input
              ...
              onChange={(e) => run(updateEventChecklistTask, { task_id: task.id, due_date: e.target.value })}
              disabled={pending}
            ...
            <select
              value={task.ownerId ?? ''}
              onChange={(e) =>
                run(updateEventChecklistTask, { task_id: task.id, task_title: task.title, owner_id: e.target.value })
              }
              disabled={pending}
            ...
            <button
              type="button"
              onClick={() => run(deleteEventChecklistTask, { task_id: task.id })}
              disabled={pending}
