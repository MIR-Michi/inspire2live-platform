...
export function CampusBriefingPanel({
  ...
  // Briefing produced this session — used to display the result immediately, and
  // as a fallback when the database can't persist it yet (migration not applied).
  const [localBriefing, setLocalBriefing] = useState<CampusBriefing | null>(null)
  ...
  const effectiveBriefing = localBriefing ?? briefing
  // Local (ephemeral) briefings can always be regenerated by the person who just
  // made one; persisted briefings are admin-only to regenerate.
  const canRegenerate = isAdmin || Boolean(localBriefing && !briefing)
  const showForm = !effectiveBriefing || editing
  ...
  const generate = () => {
    ...
    startTransition(async () => {
      ...
      if (result.briefing) setLocalBriefing(result.briefing)
  ...
  return (
    <div className="space-y-4">
      ...
      {!pending && showForm && (
        <div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <span aria-hidden>✨</span> {effectiveBriefing ? 'Regenerate briefing' : 'Generate briefing'}
            </button>
            {effectiveBriefing && editing && (
              <button
                type="button"
                onClick={() => {
                  setEditing(false)
      ...
      {!pending && effectiveBriefing && !editing && (
        <article className="space-y-4">
          <header className="space-y-1">
            <h3 className="text-base font-semibold leading-snug text-neutral-900">{effectiveBriefing.headline}</h3>
            {generatedLabel && !localBriefing && <p className="text-[11px] text-neutral-400">Generated {generatedLabel}</p>}
          </header>

          {effectiveBriefing.presenterIntro && (
            <p className="text-sm leading-6 text-neutral-700">{effectiveBriefing.presenterIntro}</p>
          )}

          {effectiveBriefing.sections.map((section, index) => (
          ...
          {effectiveBriefing.keyTakeaways.length > 0 && (
            <section className="rounded-lg border border-neutral-200 bg-neutral-50 p-3">
              ...
              <ul className="mt-2 list-disc space-y-1 pl-4 text-sm leading-6 text-neutral-700">
                {effectiveBriefing.keyTakeaways.map((item, index) => (
